...
def per_pkt_transmission(args, MM, TransmittedSymbols):
    ...
    # compute the received signal power and add noise（为保证后续代码运行，应保留）
    LL = len(TransmittedSymbols[0])
    ...
    if args.Estimator == 2:
        ...
        Cz_csr = csr_matrix(Cz)
        ## ------------------------------------- ML
        D_array_csr = csr_matrix(HH)               # 稀疏矩阵求解：5000x5000~2.7s
        d_inv_csr = srlg.inv(D_array_csr.conj().T@srlg.inv(Cz_csr)@D_array_csr)
        restored = d_inv_csr @ D_array_csr.conj().T @ srlg.inv(Cz_csr) @ samples.T  
        # if donot use noise corr. mat, get rid of all Cz_csr term
        ## ------------------------------------- Estimate SUM
...
def test():
    ...
    # loaded_image = Image.open('../test_images/d_data.png').convert('L')
    ...
    MSE1 = np.mean(np.power(np.abs(output.flatten() - target.flatten()),2))
    print('MSE1 = ', MSE1)

    # MSE of the ML estimator
    args.Estimator = 2
    output = per_pkt_transmission(args, MM, TransmittedSymbols.copy())
    MSE2 = np.mean(np.power(np.abs(output.flatten() - target.flatten()),2))
    print('MSE2 = ', MSE2)

    # MSE of the SP-ML estimator
    args.Estimator = 3
    output = per_pkt_transmission(args, MM, TransmittedSymbols.copy())
    MSE3 = np.mean(np.power(np.abs(output.flatten() - target.flatten()),2))
    print('MSE3 = ', MSE3)
